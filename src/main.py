...
def run_full_pipeline(
    ...
    """
    Run the full HUMAnN4 processing and analysis pipeline.
    
    Args:
        sample_key: Path to CSV file with sample metadata
        pathway_dir: Directory containing HUMAnN4 pathway files
        gene_dir: Directory containing HUMAnN4 gene family files
    ...
    log_print("Starting HUMAnN4 Analysis Pipeline", level="info")
    ...
    if not skip_pathway and valid_path_samples:
        ...
        if skip_pathway:
            log_print("Skipping HUMAnN4 pathway processing", level="info")
        else:
    ...
    if not skip_gene and valid_gene_samples:
        ...
        if skip_gene:
            log_print("Skipping HUMAnN4 gene processing", level="info")
        else:
    ...
    if skip_downstream:
        ...
        if not pathway_unstrat_file and not gene_unstrat_file:
            log_print("No unstratified HUMAnN4 outputs found; cannot run downstream analysis", level="warning")
            success = False
        else:
            try:
...
def run_preprocessing_and_analysis(
    ...
    KneadData → HUMAnN4 → HUMAnN4 processing → Downstream analysis.
    ...
    Args:
        ...
        nucleotide_db: Path to HUMAnN4 nucleotide database
        protein_db: Path to HUMAnN4 protein database
    ...
    # Step 1: Run preprocessing (KneadData + HUMAnN4)
    ...
    # Extract paths to HUMAnN4 output files
...
def process_humann4_files_only(
    ...
    """
    Run only the HUMAnN4 processing stage (no downstream analysis).
...
def analyze_existing_humann4_files(
    ...
    """
    Run only the downstream analysis on existing HUMAnN4 unstratified files.
    ...
    # Setup logging
    logger = setup_logger(log_file=log_file)
    log_print("Starting downstream analysis of existing HUMAnN4 files", level="info")
    start_time = time.time()
